const addVehicle = (ecocontract, accounts) => {
  ...
  $("#addvehicle").on("click", async (e) => {
    ...
    e.preventDefault();
    await ecocontract.methods.addVehicle(_registrationNo,_obdId, _vehicleType,_yearOfManufacture,_fuelType)
...
async function ecoFunctionApp() {
  ...
  const ecocontract = await getEcoContract(web3);     //Crowdsale Contract
  console.log("ecocontract", ecocontract);
  addVehicle(ecocontract, accounts); // should be called by customer
  buyInsurance(ecocontract,accounts); /// You guys have to do this
}
